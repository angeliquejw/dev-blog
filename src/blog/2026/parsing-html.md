...
Overall, this was a fun diversion, and I'm likely to take the next steps to automate the Storygraph script, too (albeit on a different schedule). You can see the result from both of these things on my [`/now` page](http://localhost:8080/now/), which includes my current reads and my most recent sky photo, plus [this new page](/bits/the-sky) which has a dated list of all of the photos.